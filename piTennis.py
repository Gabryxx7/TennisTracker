...
#Mi creao l'oggetto piCamera per accedere alla camera e setto framerate, risoluzione e spazio colori
camera = PiCamera();
...
history = 450   #Numero di frame di cui calcolare il modello del background
bgRate = 0.001; #Learning rate del metodo di sottrazione del background

#Creo una finestra con delle trackbar per calibrare l'applicazione
cv2.namedWindow("Offset");
cv2.createTrackbar("Offset X", "Offset", 0, 500, nothing);
cv2.createTrackbar("Offset Y", "Offset", 0, 500, nothing);
cv2.createTrackbar("Size P1", "Offset", 2500, 5000, nothing);
cv2.createTrackbar("Size P2", "Offset", 300, 5000, nothing);
cameraShaking = False;   #Variabile che mi dice quando il background cambia troppo
useMOG = True; #Decide quale dei due metodi usare, MOG o MOG 2
